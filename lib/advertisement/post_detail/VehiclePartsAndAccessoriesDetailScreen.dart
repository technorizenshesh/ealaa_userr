...
class VehiclePartsAndAccessoriesDetailScreen extends StatefulWidget {
  String ads_post;
  String ads_post_id;
  String user_id_value;

  VehiclePartsAndAccessoriesDetailScreen(
      {super.key, required this.ads_post, required this.ads_post_id,this.user_id_value = ''});

  @override
  State<VehiclePartsAndAccessoriesDetailScreen> createState() => _VehiclePartsAndAccessoriesDetailScreenState();
}

class _VehiclePartsAndAccessoriesDetailScreenState extends State<VehiclePartsAndAccessoriesDetailScreen> {
  List<Map<String, String>> detailElements = [
    {'title': 'Part Name', 'Image': 'assets/icons_for_car/ic_ad.png'},
    {'title': 'Condition', 'Image': 'assets/icons_for_car/ic_ad.png'},
    {'title': 'Ad Type', 'Image': 'assets/icons_for_car/ic_ad.png'},
    {'title': 'Brand', 'Image': 'assets/icons_for_car/ic_ad.png'},
    {'title': 'Quantity', 'Image': 'assets/icons_for_car/ic_ad.png'},
              ...
              children: [
                Expanded(
                  child: GestureDetector(
                    onTap: () async {
                      final Uri url = Uri(scheme: 'tel', path: result?.usersDetails?.mobile);
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () {
                      if (result!=null && result?.usersDetails?.id!=null && result!.usersDetails!.id!.isNotEmpty ||
                          result!=null && result?.usersDetails?.userName!=null && result!.usersDetails!.userName!.isNotEmpty ||
                          result!=null && result?.usersDetails?.image!=null && result!.usersDetails!.image!.isNotEmpty) {
                        push(
                            context: context,
                            screen: AdChatRoom(
                    ...
                    children: [
                      Container(
                        ...
                        child: CachedNetworkImage(
                          imageUrl: result?.vehiclePartImage ?? '',
                          fit: BoxFit.contain,
                          height: 300,
  ...
  String getTextMethod({required int index}) {
    switch (index) {
      case 0:
        return result?.partName??'NA';
      case 1:
        return result?.subPartName??'NA';
      case 2:
        return result?.makername??'NA';
      case 3:
        return result?.modelname??'NA';
      case 4:
        return result?.vehiclePartQuantity??'NA';
      case 5:
        return result?.modelTrimName??'NA';
      case 6:
        return result?.stateName??'NA';
      case 7:
        return result?.cityName??'NA';
      case 8:
        return result?.vehiclePartPrice??'NA';
      case 9:
        return result?.vehiclePartPhone??'NA';
      case 10:
        return result?.adsCreatedAt??'NA';
      default:
        return 'test';
    }
  }
